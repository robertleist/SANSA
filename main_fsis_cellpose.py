...
from models.sansa.inst_sansa_cellpose import build_cellpose_sansa
from eval_fsis_cellpose import eval_cellpose
from engine_fsis_cellpose import train_one_epoch
import opts
import mlflow
...
def main(args):
    ...
    if rank == 0:
        mlflow.set_experiment("FSIS_CELLPOSE")
        mlflow.start_run(run_name=f"SANSA_CELLPOSE - {args.name_exp}")
        mlflow.log_params(args.__dict__)

    print(args)

    device = torch.device(args.device)
    model = build_cellpose_sansa(args.sam2_version, args.adaptformer_stages, args.channel_factor, args.device)
    model.to(device)
    ...
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs * len(data_loader_train))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(model, data_loader_train, optimizer, device, epoch, args.clip_max_norm, lr_scheduler=lr_scheduler, args=args)
        ...
        if args.output_dir:
            ...
            for checkpoint_path in checkpoint_paths:
                utils.save_on_master({'model': adapter_state_dict(model_without_ddp), 'optimizer': optimizer.state_dict(), 'lr_scheduler': lr_scheduler.state_dict(), 'epoch': epoch, 'args': args}, checkpoint_path)

            print("Start validation")
            eval_stats = eval_cellpose(model, args)
            mlflow.log_metrics(eval_stats, step=epoch)

        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()}, 'epoch': epoch, 'n_parameters': n_parameters_tot}
        print(json.dumps(log_stats))


if __name__ == '__main__':
    parser = argparse.ArgumentParser('Instance SANSA Cellpose training', parents=[opts.get_args_parser_fsis()])
