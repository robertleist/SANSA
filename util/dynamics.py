...
def get_masks_torch(pt, inds, shape0, pad=20, max_size_fraction=0.4):
    ...
    Parameters:
        pt (float32, 3D or 4D array): Final locations of each pixel after dynamics,
            size [axis x Ly x Lx] or [axis x Lz x Ly x Lx].
        pad (int, optional): Histogram edge padding. Default is 20.
...
def compute_masks(dP, cellprob, p=None, niter=200, cellprob_threshold=0.0,
    ...
    if (cellprob > cellprob_threshold).sum():  #mask at this point is a cell cluster binary map, not labels
        indices = np.nonzero(cellprob > cellprob_threshold)
        if len(indices[0]) == 0:
        ...
        mask = get_masks_torch(p_final, indices, dP.shape[1:],
                               max_size_fraction=max_size_fraction)
        del p_final
